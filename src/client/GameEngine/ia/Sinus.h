...
public:
	...
	virtual IArtificialIntelligence *NewIA(const uint32_t myX, const uint32_t myY, const uint32_t width, const uint32_t height);

	Sinus & operator=(Sinus *);

	virtual Action actualize(std::vector<AIPosition>&, std::vector<AIPosition>& enemies, AIPosition myPos) override;
