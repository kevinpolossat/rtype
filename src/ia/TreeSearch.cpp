...
const double MCTS::Node::calculUCB() const
{
  ...
  if (_visit == 0)
    return (42);
  double k = static_cast<double>(_score) / static_cast<double>(_visit);
  k += 2 * log(static_cast<double>(_parent->getVisits() * 1000) / static_cast<double>(_visit));
  return k;
}
...
MCTS::Node *MCTS::TreeSearch::expandNode(MCTS::Node *x)
{
  ...
  if (isInMap(p))
  {
    bool shot = false;
    for (auto s : _shoots)
      if (s->collisionNoAutoIncrease(p, 0, 2))
        shot = true;
    if (!shot)
...
const int MCTS::TreeSearch::simulateNode(MCTS::Node *x)
{
  ...
  for (int i = 0; i < _simuDepth; i++)
  {
    for (auto s : _shoots)
    {
      if (s->collisionNoAutoIncrease(testPosition, 0, 2))
        return dead;
      s->increaseShoot();
    }
    dead++;
