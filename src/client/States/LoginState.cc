...
bool LoginState::Init(ge::GameEngine & engine) {
	...
	world_.CreateText(ge::Vector2f(size.x / 2 - 50, size.y / 5.f * 2), "Create", "retro", CREATE);
	world_.CreateText(ge::Vector2f(size.x / 2 - 50, size.y / 5.f * 3), "Join", "retro", JOIN);
	world_.CreateText(ge::Vector2f(size.x / 2 - 50, size.y / 5.f * 4), "Cancel", "retro", CANCEL);
